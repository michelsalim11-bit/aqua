...
export function PageHero({
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-white/5" aria-labelledby={titleId}>
      ...
      <Container className={`relative flex flex-col gap-5 pb-16 pt-20 md:gap-6 md:pb-28 md:pt-32 ${alignClass}`.trim()}>
        <span className="inline-flex w-fit items-center gap-2 rounded-full border border-aqua-green/30 bg-aqua-green/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-aqua-green sm:text-xs">
          {eyebrow}
        </span>
        <h1
          id={titleId}
